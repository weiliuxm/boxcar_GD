...
from boxcars_image_transformations import alter_HSV, image_drop, unpack_3DBB, add_bb_noise_flip, cropped_2DBB
import random
...
class BoxCarsDataGenerator(Iterator):
    ...
    def next(self):
        ...
        for i, ind in enumerate(index_array):
            ...
            vehicle, instance, bb3d, bb2d = self.dataset.get_vehicle_instance_data(vehicle_id, instance_id)
           ...
           # image = unpack_3DBB(image, bb3d)
            image = cropped_2DBB(image,bb2d) 
            image = (image.astype(np.float32) - 116)/128.
            x[i, ...] = image
        if not self.generate_y:
            return x
 #       print(self.dataset.Y[self.part].keys())
  

        dic = {}      
        for key,value in self.dataset.Y[self.part].items():
            dic[key] = value[index_array]
